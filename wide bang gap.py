...
layers 			= ['SiO2','ITO' , 'nio','lowbgperov', 'c60','aldsno2','gold']
thicknesses		= [0 ,100,50,350,50,15,100]
#
...
if plotGeneration:
	...
	for matind in np.arange(2,len(t)+1):
		...
		ax3.text(xpos,sorted(Gx2[0])[0],layers[matind-1],va='bottom',ha='center')
	fig3.show()
	
	# calculate Jsc
	Jsc = np.sum(Gx2)*x_step*1.0e-7*q*1.0e3
	#g1=np.sum(Gx1)*x_step*1.0e-7
	g2=np.sum(Gx2)*x_step*1.0e-7
	
	# calculate parasitic absorption
	parasitic_abs = (1.0 - Reflection - Absorption[activeLayer2,:])

...
layers 			= ['SiO2','ITO' , 'pedotpss','lowbgperov', 'c60','aldsno2','gold']
thicknesses		= [0 ,100,50,950,20,15,100]
#
plotGeneration	= True		# Make generation plot , True/False
activeLayer1		= 	3		# indexing starts from 0
#activeLayer2		=	6
...
if plotGeneration:
	...
	for l in am15_data:
		...
		am15_xData.append(x)
		am15_yData.append(y)
	am15_interp = interp1d(am15_xData,am15_yData,'linear',fill_value="extrapolate")
	...
	ax3.plot(x_pos[ActivePos1[0]] , Gx1[0])
	#ax3.plot(x_pos[ActivePos2[0]] , Gx2[0])
	# Layer Bars
	for matind in np.arange(2,len(t)+1):
		xpos = (t_cumsum[matind-2]+t_cumsum[matind-1])/2.0
		ax3.axvline(np.sum(t[0:matind]),linestyle=':')
		ax3.text(xpos,sorted(Gx1[0])[0],layers[matind-1],va='bottom',ha='center')
	fig3.show()
	
	# calculate Jsc
	Jsc = np.sum(Gx1)*x_step*1.0e-7*q*1.0e3
	g1=np.sum(Gx1)*x_step*1.0e-7
	#g2=np.sum(Gx2)*x_step*1.0e-7
	
	# calculate parasitic absorption
	parasitic_abs = (1.0 - Reflection - Absorption[activeLayer1,:])
	print("maximum generation rate in (/cm^3 sec) \n",max(Gx1[0]))
